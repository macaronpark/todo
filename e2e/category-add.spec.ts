...
test('새 카테고리를 추가하면 제목을 바로 수정할 수 있는 상태로 보여진다.', async ({
  mainPage,
}) => {
...
test('새 카테고리를 추가하면 태스크 섹션에 해당 카테고리가 선택되어 보여진다.', async ({
  mainPage,
}) => {
  await mainPage.addCategory();
...
test('새 카테고리 제목을 초기 제목과 동일한 상태로 Enter로 확정하면 카테고리 제목은 `새 카테고리`로 보여진다.', async ({
  mainPage,
}) => {
  await mainPage.addCategory();
...
test('새 카테고리 제목을 수정하고 Enter로 확정하면 카테고리 제목은 수정한 제목으로 보여진다.', async ({
  mainPage,
}) => {
  await mainPage.addCategory();
